...
public class StartCSE360 extends Application {
        ...
        submitButton.setOnAction(e -> {
            try {
                if (databaseHelper.login(emailField.getText(), passwordField.getText(),Role.student)) {
                    ...
                    showInstructorOptions(primaryStage,emailField.getText());
                }else {
       ...
       // User user = null;
        ...
        viewButton.setOnAction(e -> {
            ...
            try {
                DatabaseHelper_Encryption dbHelper = new DatabaseHelper_Encryption();
                dbHelper.connectToDatabase();
                String article = dbHelper.getArticleById(Long.parseLong(id), role);
                dbHelper.closeConnection();
    ...
    public static void showListByGroupScreen(Stage primaryStage, int bool) {
        ...
        backButton.setOnAction(e ->{
        	if(bool == 1) {
        		...
        		showInstructorOptions(primaryStage,"");
        	}
        });
    ...
    static void showArticleSearchScreen(Stage primaryStage, String email, Role role) {
        ...
        // Pass the `role` parameter when calling `performArticleSearch`
        searchButton.setOnAction(e -> performArticleSearch(primaryStage, titleField.getText(), authorsField.getText(), keywordsField.getText(), IDField.getText(), email, role));

        Button backButton = new Button("Back");
        backButton.setOnAction(e -> {
            // Navigate back to the appropriate screen based on the user's role
            switch (role) {
                case student:
                    showStudentOptions(primaryStage, email);
                    break;
                case instructor:
                    showInstructorOptions(primaryStage, email);
                    break;
                default:
    ...
    private static void performArticleSearch(Stage primaryStage, String title, String authors, String keywords, String ID, String email, Role role) {
        try {
            ...
            Button searchButton = new Button("Search");
            searchButton.setOnAction(e -> showArticleSearchScreen(primaryStage, email, role));
    ...
    private static void displayArticleScreen(Stage primaryStage, List<Article> articles, String serialNumber, String email) {
        ...
        int serialNumber1;
        try {
            serialNumber1 = Integer.parseInt(serialNumber);
        } catch (NumberFormatException e) {
            showErrorDialog("Input Error", "Please enter a valid sequence number.");
            return;
        }

        // Ensure serialNumber1 is within valid range
        if (serialNumber1 <= 0 || serialNumber1 > articles.size()) {
            showErrorDialog("Error", "Serial number out of range");
            return;
        }

        // Get the selected article
        Article selectedArticle = articles.get(serialNumber1 - 1);
        ObservableList<Article> articleList = FXCollections.observableArrayList(selectedArticle);
        ...
        refColumn.setCellValueFactory(new PropertyValueFactory<>("references"));
        ...
        Button backButton = new Button("Back");
        backButton.setOnAction(e -> showArticleSearchScreen(primaryStage, email, Role.student)); // Update navigation logic

        Button searchButton = new Button("Search");
        searchButton.setOnAction(e -> showArticleSearchScreen(primaryStage, email, Role.student));
    ...
    public static void showStudentOptions(Stage primaryStage, String email) {
        ...
        searchArticleButton.setOnAction(e -> {
            showArticleSearchScreen(primaryStage, email, Role.student);
        });
    ...
    public static void showInstructorOptions(Stage primaryStage, String email) {
        VBox instructorLayout = new VBox(30);
        instructorLayout.setStyle("-fx-padding: 10;");

        Label mainLabel = new Label("Instructor Options");
        Button createArticleButton = new Button("Create an Article");
        Button viewArticleByID = new Button("View article by ID");
        Button displayAllArticlesButton = new Button("Display all articles");
        Button deleteAnArticleButton = new Button("Delete an article");
        Button restoreArticleButton = new Button("Restore to a file");
        Button backupArticleButton = new Button("Backup from an existing file");
        Button listArticlesByGroup = new Button("List articles by group");
        Button searchArticleButton = new Button("Search an Article");
        Button addStudentToGroup = new Button("Add student to special access group");
        Button logoutButton = new Button("Logout");
        
        searchArticleButton.setOnAction(e -> {
            showArticleSearchScreen(primaryStage, email, Role.instructor);
        });

        
        createArticleButton.setOnAction(e->{
        	createArticle(primaryStage,0);
        });
        
        viewArticleByID.setOnAction(e->{
        	viewArticleById(primaryStage,0);
        });
        
        displayAllArticlesButton.setOnAction(e->{
        	displayAllArticles(primaryStage,0);
        });
        
        deleteAnArticleButton.setOnAction(e->{
        	deleteArticle(primaryStage,0);
        });
        
        restoreArticleButton.setOnAction(e->{
        	restoreArticles(primaryStage,0);
        });
        
        backupArticleButton.setOnAction(e->{
        	backupArticles(primaryStage,0);
        });
        
        listArticlesByGroup.setOnAction(e->{
        	showListByGroupScreen(primaryStage,0);
        });
                
        addStudentToGroup.setOnAction(e->{
        	addStudentToGroup(primaryStage);
        });
        
        logoutButton.setOnAction(e->{
        	showMainLayout(primaryStage);
        });
        instructorLayout.getChildren().addAll(mainLabel,createArticleButton, viewArticleByID,displayAllArticlesButton,deleteAnArticleButton, restoreArticleButton, backupArticleButton, listArticlesByGroup,searchArticleButton, addStudentToGroup,logoutButton);
        primaryStage.setScene(new Scene(instructorLayout, 800, 600));
    }    
