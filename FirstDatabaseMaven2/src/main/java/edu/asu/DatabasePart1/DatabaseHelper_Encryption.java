...
class DatabaseHelper_Encryption {
    ...
    public void addArticle(String[] articleArray) throws Exception {
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(insertArticle)) {
            ...
            pstmt.setString(7, articleArray[6]);  // Encrypted body
    ...
    public String getArticleById(long id) throws Exception {
    ...
    // Retrieve article details by sequence number for backup purposes
//    public String[] backUp(int sequenceNumber) throws Exception {
//        String sql = "SELECT * FROM cse360articles WHERE sequence_number = ?";
//        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
//            pstmt.setInt(1, sequenceNumber);  // Set the sequence number parameter
//            ResultSet rs = pstmt.executeQuery();
//
//            // If the article exists, decrypt and return the article data as an array
//            if (rs.next()) {
...
//                String references = rs.getString("referencesColumn");
//
//                // Decrypt the body of the article
//                String decryptedBody = "No body available";  // Default value if no body is found
//                if (encryptedBody != null && !encryptedBody.isEmpty()) {
//                    try {
//                        char[] bodyChars = EncryptionUtils.toCharArray(
//                            encryptionHelper.decrypt(
//                                Base64.getDecoder().decode(encryptedBody),
//                                EncryptionUtils.getInitializationVector(title.toCharArray())
//                            )
//                        );
//                        decryptedBody = new String(bodyChars);
//                        Arrays.fill(bodyChars, '0');  // Clear sensitive data from memory
//                    } catch (Exception e) {
//                        System.out.println("Error decrypting body for article ID: " + id);
//                    }
//                }
//
//                // Populate array with article details and return it
//                array[0] = String.valueOf(sequenceNumber);
//                array[1] = Long.toString(id);
//                array[2] = title;
//                array[3] = authors;
//                array[4] = ab;
//                array[5] = keywords;
//                array[6] = decryptedBody;
//                array[7] = references;
//
//                return array;  // Return article data
//            }
//        }
